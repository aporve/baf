window.addEventListener('message', function (eventData) {
  ...
  if (parsedEventData.event_code === "custom-recent-order-event") {
    parent.postMessage(JSON.stringify({
      ...
      data: JSON.stringify({
        event: {
          code: "custom-parenttoroot-recent-order-event",
          data: parsedEventData.data,
        },
      }),
    }), '*');
  ...
  if (parsedEventData.event_code === "custom-select-option-1") {
    parent.postMessage(JSON.stringify({
      ...
      data: JSON.stringify({
        event: {
          code: "custom-parenttoroot-select-option-1",
          data: parsedEventData.data,
        },
      }),
    }), '*');
  ...
  if (parsedEventData.event_code === "custom-parent-client-event" && parsedEventData.data) {
    ...
    document.querySelector("iframe").contentWindow.postMessage(JSON.stringify({
      ...
      data: JSON.stringify({
        event: {
          code: "custom-child-client-event",
          data: parsedEventData.data,
        },
      }),
    }), '*');
